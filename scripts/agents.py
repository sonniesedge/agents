...
FILE_KINDS = ("config",)
# `rules` is either: a directory links each themed file, a path ending in
# .md concatenates them into one generated file.
DUAL_KINDS = ("rules",)
...
def targets_a_file(kind: str, target: Path) -> bool:
    """Whether this target is a single file rather than a directory."""
    if kind in FILE_KINDS:
        return True
    return kind in DUAL_KINDS and target.suffix == ".md"
...
def rule_files() -> list[Path]:
    """Themed rule files, in the order they are concatenated or linked."""
    src = REPO / "rules"
    if not src.is_dir():
        return []
    return [f for f in sorted(src.glob("*.md")) if f.name != "README.md"]


def render_rules(filename: str) -> Path | None:
    """Concatenate rules/*.md into one file, for tools given a file target.

    Order is by filename, so numeric prefixes (10-git.md) control it. Each
    section is labelled with its source; Claude Code strips block-level HTML
    comments before loading, so the label costs no context there.
    """
    out_dir = BUILD / "rules"
...
def prune_stale_targets(cfg: Config) -> None:
    ...
    current = {str(t) for kinds in cfg.targets.values() for t in kinds.values()}
...
def link(cfg: Config, built: Path) -> None:
    ...
    for tool, kinds in cfg.targets.items():
        for kind, target in kinds.items():
            ...
            if kind in DUAL_KINDS and targets_a_file(kind, target):
                link_file(target, render_rules(target.name))
...
def check_rules_wiring(tool: str, kinds: dict[str, Path]) -> None:
    ...
    rules = kinds["rules"]
    # A concatenated file is read natively (AGENTS.md); only a directory of
    # separate files has to be globbed from the config.
    if targets_a_file("rules", rules):
        return
